...
/**
 * 자주 묻는 질문 메인 페이지를 불러오는 클래스
 * @author 허주원
 *
 */
@WebServlet("/question/questionmain.do") // web.xml을 작성 안해도됨
public class QuestionMain extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (n == 1) {
			...
			pagebar += String.format("<div>\r\n"
					+ "					<button type=\"button\" class=\"btn_image\" id=\"img_btn_left\" onclick=\"location.href='/web_team6/question/questionmain.do?page=%d'\">\r\n"
					+ "						<img src=\"/web_team6/asset/images/chevron-left.svg\">\r\n"
		...
		while (!(loop > blockSize || n > totalPage)) {
			pagebar += String.format(
					"<a href=\"/web_team6/question/questionmain.do?page=%d\" rel=\"0\" class=\"\">%d</a> ", n, n);
		...
		if (n > totalPage) {
			...
			pagebar += String.format("</div><div>\r\n"
					+ "					<button type=\"button\" class=\"btn_image\" id=\"img_btn_right\" onclick=\"location.href='/web_team6/question/questionmain.do?page=%d'\">\r\n"
					+ "						<img src=\"/web_team6/asset/images/chevron-right.svg\">\r\n"
					+ "					</button>\r\n" + "				</div>", n);
		}
		...
		ArrayList<QuestionDTO> qList = dao.questionList(begin, end);

		req.setAttribute("totalCount", totalCount);
		req.setAttribute("qList", qList);
		req.setAttribute("pagebar", pagebar);

		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/question/questionmain.jsp");
